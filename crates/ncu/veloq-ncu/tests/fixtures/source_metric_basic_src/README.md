...
cache ships alongside the fixture, together with the companion
`<sha>.cubin` it is keyed from. The disasm pipeline
...
`.gitignore` rule. The companion `<sha>.cubin` is also
committed: when the `.ncu-rep` itself is absent, `disasm`
loads committed cubins from the same sidecar directory before
it can derive the SHA and hit the correlated cache.
...
git add -f \
    crates/ncu/veloq-ncu/tests/fixtures/source_metric_basic.ncu-rep.veloq/ncu-native.json.gz \
    crates/ncu/veloq-ncu/tests/fixtures/source_metric_basic.ncu-rep.veloq/disasm/*.cubin \
    crates/ncu/veloq-ncu/tests/fixtures/source_metric_basic.ncu-rep.veloq/disasm/*.correlated.json
```
